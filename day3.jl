...
inputs = read_to_array("inputs/3_1", String)
...
# (called the gamma rate and the epsilon rate). The power consumption can then be found by multiplying the gamma rate 
# by the epsilon rate.
...
# the diagnostic report. For example, given the following diagnostic report:
...
function day3_1(inputs)
    ...
    gamma = parse(UInt, gamma_string, base=2)
    epsilon = parse(UInt, epsilon_string, base=2)

    println("Gamma String: $gamma_string, Gamma: $gamma, Epsilon: $epsilon")
    gamma * epsilon
end
...
println("Test 3-1: $(day3_1(test))")
println("Day 3-1: $(day3_1(inputs))")
...
function day3_2(inputs)
    ...
    keep = Array{Char}(undef, length(counts))
    fill!(keep, '0')
    ...
    for i in 1:length(keep)
        if length(remaining_oxygen) > 1
            for o in remaining_oxygen
                if o[i] != keep[i]
                    delete!(remaining_oxygen, o)
                end
            end
        end

        if length(remaining_c02) > 1
            for c in remaining_c02
                if c[i] == keep[i]
                    delete!(remaining_c02, c)
                end
            end
        end
    end

    oxygen = parse(UInt, set_pop(remaining_oxygen), base=2)
    c02 = parse(UInt, set_pop(remaining_c02), base=2)
...
println("Test 3-2: $(day3_2(test))")
# println("Day 3-2: $(day3_2(inputs))")
